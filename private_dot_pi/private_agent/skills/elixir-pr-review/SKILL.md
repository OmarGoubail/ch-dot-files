...
5. Read global and repo review memory through the QMD-backed `review_memory` tool when present. Memory is a verification lens, never proof; ordinary reviews are read-only. Do not append patterns or write reflections unless `/elixir-review learn` or another explicit persistence request authorizes it.
